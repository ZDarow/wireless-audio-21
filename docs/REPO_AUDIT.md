...
|---|-----------|----------|-----|------------|
| V1 | **ESP-NOW без шифрования** (`encrypt=false`, LMK/PMK не заданы) | 🔴 Высокий | `espnow.h:34,84` | По докам Espressif: «без LMK кадры не шифруются». Задать `esp_now_set_pmk()`, каждому пиру `lmk` + `encrypt=true` (лимит encrypt-пиров ≥7 — для 2 сателлитов достаточно) |
| V2 | **Хэш пароля SHA-256(пароль + фиксированная публичная соль), без итераций; пароль ≥ 4 символа** | 🟠 Средний | `auth.h:135-149`, `web_server.h:901` | Использовать PBKDF2-HMAC-SHA256 (≥1000 итераций) с per-device солью из `esp_random` (соль хранить в NVS рядом); мин. длину пароля поднять до 8; ограничить количество попыток (уже есть — C5.3) |
...
- [ ] S3 — добавить `LICENSE`.
- [ ] S7 — активировать GitHub Actions (Settings → Actions).
- [ ] V1 — шифрование ESP-NOW (PMK/LMK, `encrypt=true`).
- [ ] V2 — PBKDF2 + per-device соль; мин. пароль 8 символов.

### Приоритет 1 (безопасность/стабильность)
- [ ] V4 — аутентификация UDP-источника (токен/remoteIP).
- [ ] S6 — CI: `permissions`, `timeout-minutes`.
...
- [ ] S14 — перенос ТЗ в `docs/`, удаление мусора.
